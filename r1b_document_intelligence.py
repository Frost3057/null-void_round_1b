...
def extract_page_text_content_with_fallback(document_page, pdf_document_path, page_index):
    ...
    if validate_character_encoding_integrity(extracted_text, detected_language):
        try:
            ...
            image_conversion_result = convert_from_path(pdf_document_path, first_page=page_index+1, last_page=page_index+1)
            extracted_text = ""
        except Exception:
            extracted_text = ""
...
def execute_advanced_document_intelligence_pipeline():
    ...
    semantic_embedding_model = SentenceTransformer('./multilingual_model/', device='cpu')
    combined_query_text = researcher_persona_definition + ' ' + research_task_description
    query_vector_embedding = semantic_embedding_model.encode(combined_query_text, show_progress_bar=False, convert_to_numpy=True, normalize_embeddings=True)
    ...
    for pdf_document_path in available_pdf_documents:
        ...
        for page_index, document_page in enumerate(pdf_document):
            extracted_text, detected_language = extract_page_text_content_with_fallback(document_page, pdf_document_path, page_index)
    ...
    for section_item in all_extracted_sections:
        ...
        section_vector_embedding = semantic_embedding_model.encode(aggregated_section_text, show_progress_bar=False, convert_to_numpy=True, normalize_embeddings=True)
        computed_semantic_relevance_score = float(np.dot(query_vector_embedding, section_vector_embedding))
        sections_with_semantic_scores.append({
            **section_item,
            "relevance_score": computed_semantic_relevance_score
        })
    priority_ranked_sections = sorted(sections_with_semantic_scores, key=lambda section: section['relevance_score'], reverse=True)
    ...
    json_output_path = os.path.join(target_output_directory, "r1b_output.json")
    ...
    print(f"Generated R1B output: {json_output_path}")

if __name__ == "__main__":
    execute_advanced_document_intelligence_pipeline()